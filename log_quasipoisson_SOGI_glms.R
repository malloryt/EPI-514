################################################################################
# Script purpose: Survey weighted glm for secondary analyses with SOGI module
...
year_by_state = table(BRFSS_merged$state_name,BRFSS_merged$BRFSS_YEAR,
                      is.na(BRFSS_merged$TRNSGNDR)|is.na(BRFSS_merged$SO)) %>% 
  as.data.frame()
missing_year = year_by_state %>% filter(Freq==0 & Var3==FALSE) %>%
  select(Var1) %>% unique()

`%!in%` = Negate(`%in%`)

complete_data = BRFSS_merged %>% filter(state_name %!in% missing_year$Var1)

## Filter to relevant variables ##
subset = complete_data[,!c("X_RACE","X_URBSTAT","SOMALE","SOFEMALE","BIRTHSEX",
                           "HIVTST6","HIVTSTD3")]
...
FB_P_results = svyglm(HIV_test~MEDCOST*COV_YEAR+HLTHPLN1+SEX+X_MRACE1+X_HISPANC+
                   X_AGEG5YR+INCOME2+EMPLOY1+RENTHOM1+X_EDUCAG+SO+TRNSGNDR,
                 design = design, family = quasipoisson(link="log"))
save(FB_P_results,file="FB_P_results_SOGI.rds")
...
HC_P_results = svyglm(HIV_test~HLTHPLN1*COV_YEAR+MEDCOST+SEX+X_MRACE1+X_HISPANC+
                        X_AGEG5YR+INCOME2+EMPLOY1+RENTHOM1+X_EDUCAG+SO+TRNSGNDR,
                      design = design, family = quasipoisson(link="log"))
save(HC_P_results,file="HC_P_results_SOGI.rds")
...
results = svyglm(HIV_test~MEDCOST+HLTHPLN1+COV_YEAR+SEX+X_MRACE1+X_HISPANC+
                        X_AGEG5YR+INCOME2+EMPLOY1+RENTHOM1+X_EDUCAG+SO+TRNSGNDR,
                      design = design, family = quasipoisson(link="log"))
save(results,file="results_SOGI.rds")
...
## Filter and create the survey design object for pre-COVID19 ##

c_pre = complete_data %>% filter(COV_YEAR=="2017-2019")

design_pre <- svydesign(data = c_pre,
                    id = ~X_PSU, strata = ~X_STSTR, weights = ~LLCPWT_5Y, nest = TRUE)
options(survey.lonely.psu = "adjust")

## Fully adjusted survey-weighted log-poisson model pre-stratum ##
results_pre = svyglm(HIV_test~MEDCOST+HLTHPLN1+SEX+X_MRACE1+X_HISPANC+
                   X_AGEG5YR+INCOME2+EMPLOY1+RENTHOM1+X_EDUCAG+SO+TRNSGNDR,
                 design = design_pre, family = quasipoisson(link="log"))
save(results_pre,file="results_SOGI_pre.rds")


## PR point estimates ##
FB_PR=exp(coef(results_pre)["MEDCOST"])
round(FB_PR,3)

HC_PR=exp(coef(results_pre)["HLTHPLN1"])
round(HC_PR,3)

### PR 95% CIs ##
FB_CI=coefci(results_pre,vcov.=vcov(results_pre))[2,] %>% exp()
HC_CI=coefci(results_pre,vcov.=vcov(results_pre))[3,] %>% exp()
round(FB_CI,3)
round(HC_CI,3)

## Filter and create the survey design object for pre-COVID19 ##

c_post = complete_data %>% filter(COV_YEAR=="2020-2021")

design_post <- svydesign(data = c_post,
                        id = ~X_PSU, strata = ~X_STSTR, weights = ~LLCPWT_5Y, nest = TRUE)
options(survey.lonely.psu = "adjust")

## Fully adjusted survey-weighted log-poisson model post-stratum ##
results_post = svyglm(HIV_test~MEDCOST+HLTHPLN1+SEX+X_MRACE1+X_HISPANC+
                       X_AGEG5YR+INCOME2+EMPLOY1+RENTHOM1+X_EDUCAG+SO+TRNSGNDR,
                     design = design_post, family = quasipoisson(link="log"))
save(results_pre,file="results_SOGI_post.rds")


## PR point estimates ##
FB_PR=exp(coef(results_post)["MEDCOST"])
round(FB_PR,3)

HC_PR=exp(coef(results_post)["HLTHPLN1"])
round(HC_PR,3)

### PR 95% CIs ##
FB_CI=coefci(results_post,vcov.=vcov(results_post))[2,] %>% exp()
HC_CI=coefci(results_post,vcov.=vcov(results_post))[3,] %>% exp()
round(FB_CI,3)
round(HC_CI,3)
